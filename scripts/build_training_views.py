...
DATA = ROOT / "data"
PILOT = ROOT / "scripts" / "build_data_pilot.py"
...
def source_views():
    features = rows(DATA / "derived" / "forecast_features.jsonl")
    labels = {x["feature_id"]: x for x in rows(DATA / "derived" / "forecast_labels.jsonl")}
    ...
    synthetic_evidence = rows(DATA / "derived" / "f1_train_corpus.jsonl")
...
def build(output):
    for command in ([sys.executable, str(PILOT), "build"], [sys.executable, str(PUBLIC), "build"]):
