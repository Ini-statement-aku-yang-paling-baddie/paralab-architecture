...
> **Terkait:** [ARCHITECTURE-V4.md](ARCHITECTURE-V4.md) §11 (F_CV: Visual Screening sebagai Asisten Capture untuk F3), [DATA-PIPELINE.md](DATA-PIPELINE.md), [cv/README.md](cv/README.md).
...
5. Taksonomi CV ternyata nyaris identik dengan enum `appearance` di dataset F3 yang sedang dikerjakan (`data/full_synthetic/`) — mapping langsung tanpa kehilangan informasi. Contoh gate confidence/abstention dan jalur human-confirmation sudah diformalkan sebagai modul **F_CV** di [ARCHITECTURE-V4.md §11](ARCHITECTURE-V4.md#11-f_cv-visual-screening-sebagai-asisten-capture-untuk-f3). Yang tersisa murni eksekusi, bukan desain: validasi domain di foto asli (belum lolos) — lihat §4.
...
Ini konsisten dengan keputusan produk yang sudah ada: [ARCHITECTURE-V4.md](ARCHITECTURE-V4.md) menaruh classifier CV visual di **P2, "Ditunda sampai memperoleh dataset gambar berlabel yang sesuai domain"** — bukan P0/P1 seperti F1/F2/F3.
...
**Catatan penting**: dataset pilot lama (`data/canonical/`) **tidak diretrofit** untuk mapping ini. Ia sudah dibangun, divalidasi, dan dikunci lewat SHA-256 manifest sesuai disiplin di [DATA-PIPELINE.md](DATA-PIPELINE.md) ("snapshot yang berbeda dari build sebelumnya ditolak, bukan ditimpa diam-diam"). Integrasi CV diarahkan ke `full_synthetic` saja, yang memang jadi target skala produksi sesuai arsitektur.
...
1. Checkpoint di arsitektur ini didesain append-only/immutable (ARCHITECTURE-V4.md §4) — begitu tersimpan, salah baca CV itu jadi bagian permanen dari record trial.
...
### Fase 0 — Selaras skema (SELESAI, sudah diformalkan di [ARCHITECTURE-V4.md §11](ARCHITECTURE-V4.md#11-f_cv-visual-screening-sebagai-asisten-capture-untuk-f3))
...
- Audit event mencatat: `model_version` CV, confidence, apakah dikonfirmasi/dikoreksi manusia (mengikuti §14.2 ARCHITECTURE-V4.md).
...
- Baru pertimbangkan menambah kelas/produk family lain setelah Fase 2-3 terbukti generalize di domain O/W gel-cream, sesuai roadmap tertunda §17 ARCHITECTURE-V4.md.
...
- Mekanisme human-confirmation dan confidence-gate/abstention sudah didesain formal ([ARCHITECTURE-V4.md §11](ARCHITECTURE-V4.md#11-f_cv-visual-screening-sebagai-asisten-capture-untuk-f3)), tapi **belum ada implementasi kode** — masih spesifikasi arsitektur, belum tersambung ke pipeline F3 nyata.
