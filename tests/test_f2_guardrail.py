...
# ================= TESTS =================
K = {'target_skin': 'oily', 'segmen': 'premium'}
def t(name, cond):
    print(('✔' if cond else '✘ FAIL'), name)
    assert cond, name

# skenario studi kasus: Squalane -> Mineral Oil 5%, + Fragrance 0.5%
f_edit = [{'bahan':'Niacinamide','pct':5},{'bahan':'Zinc PCA','pct':1},{'bahan':'Mineral Oil','pct':5},
          {'bahan':'Dimethicone','pct':2},{'bahan':'Glycerin','pct':3},{'bahan':'Panthenol','pct':1},
          {'bahan':'Fragrance "Citrus Woody Blend"','pct':0.5}]
out = screen_formula(f_edit, K, ph=5.7)
mo = next(r for r in out['results'] if r.get('inci_name') == 'Mineral Oil')
fg = next(r for r in out['results'] if r.get('inci_name') == 'Fragrance')
t('1. Mineral Oil → warning (premium positioning via konteks) ', mo['status'] == 'warning')
t('2. Fragrance → warning (syubhat halal), prefix-resolve     ', fg['status'] == 'warning')
t('3. semua warning punya rule_id + version + source_id       ',
  all('rule_id' in f and 'rule_version' in f and 'source_id' in f for r in out['results'] for f in r['rules_fired']))
t('4. overall = warning (bukan blocked)                       ', out['overall_status'] == 'warning')
t('5. requires_human_signoff = True                           ', out['requires_human_signoff'] is True)
df = out['derived_features']
t('6. derived features: electrolyte_load=high (Zinc PCA = elektrolit) ', df['electrolyte_load'] == 'high')
t('7. feature schema versioned                                ', df['feature_schema_version'] == 'stability-sentinel-v1')

# konflik aktif → blocked + fail-fast
f_conf = [{'bahan':'Ascorbic Acid','pct':10},{'bahan':'Niacinamide','pct':4}]
out2 = screen_formula(f_conf)
aa = next(r for r in out2['results'] if r.get('inci_name') == 'Niacinamide')
t('8. konflik Ascorbic×Niacinamide → BLOCKED (fail-fast)      ', aa['status'] == 'blocked')
t('9. blocked rule = COMPAT-xxx                                ', aa['rules_fired'][0]['rule_id'].startswith('COMPAT'))

# khusus obat → blocked
out3 = screen_formula([{'bahan':'Benzoyl Peroxide','pct':2.5}])
t('10. khusus obat → blocked                                   ', out3['results'][0]['status'] == 'blocked')

# bahan tak dikenal → unknown
out4 = screen_formula([{'bahan':'Ekstrak Bunga XYZ','pct':1}])
t('11. bahan tak dikenal → unknown (abstain, bukan tebak)      ', out4['results'][0]['status'] == 'unknown')

# typo STT
t('12. typo "niasinamida" → Niacinamide                        ', resolve('niasinamida') == 'ING:NIACINAMIDE')

# electrolyte-thickener risk (fitur F3)
f_et = [{'bahan':'Sodium Hyaluronate','pct':0.5},{'bahan':'Carbomer','pct':0.5},{'bahan':'Panthenol','pct':1}]
out5 = screen_formula(f_et, ph=5.5)
t('13. electrolyte_thickener_risk=high (Sodium×Carbomer)       ', out5['derived_features']['electrolyte_thickener_risk'] == 'high')

# determinisme: input sama → output sama (timestamp dikecualikan — dia memang berbeda per panggilan)
out_re = screen_formula(f_edit, K, ph=5.7)
out_re.pop('screened_at'); out_orig = {k: v for k, v in out.items() if k != 'screened_at'}
t('14. determinisme (2x screen identik, ekskl. timestamp)      ', out_re == out_orig)

print('\n🎉 14/14 acceptance tests lolos — F2 contract V4 siap.')
